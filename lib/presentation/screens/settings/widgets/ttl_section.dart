...
class _TtlSectionState extends ConsumerState<TtlSection> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() {
      _timeEnabled = cfg.timeEnabled; _timeDays = cfg.timeDays.toDouble();
      _sizeEnabled = cfg.sizeEnabled; _sizeGb = cfg.sizeGb.toDouble();
    });
  }

  Future<void> _save() async {
    await ref.read(ttlEngineProvider).updateConfig(TtlConfig(
      timeEnabled: _timeEnabled, timeDays: _timeDays.toInt(),
      sizeEnabled: _sizeEnabled, sizeGb: _sizeGb.toInt(),
    ));
  }
    ...
    return EnpixSection(
      ...
      children: [
        SwitchListTile(
          ...
          subtitle: Text(_timeEnabled ? '删除 ${_timeDays.toInt()} 天前且已上传的本地文件' : '已禁用'),
          value: _timeEnabled,
          onChanged: (v) { setState(() => _timeEnabled = v); _save(); },
        ),
        if (_timeEnabled) _buildTimeSlider(),
        const Divider(indent: AppSpacing.lg),
        SwitchListTile(
          title: const Text('按空间清理'),
          subtitle: Text(_sizeEnabled ? '本地空间超过 ${_sizeGb.toInt()} GB 时清理旧文件' : '已禁用'),
          value: _sizeEnabled,
          onChanged: (v) { setState(() => _sizeEnabled = v); _save(); },
        ),
