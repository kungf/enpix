...
void main() async {
  ...
  Future<Uint8List> enc(Uint8List p, Uint8List k, Uint8List n) async {
    final b = await aead.encrypt(p, secretKey: SecretKey(k), nonce: n);
    final r = Uint8List(n.length + b.cipherText.length + b.mac.bytes.length);
    r.setAll(0, n);
    r.setAll(n.length, b.cipherText);
    r.setAll(n.length + b.cipherText.length, b.mac.bytes);
    return r;
  }

  Future<Uint8List> dec(Uint8List d, Uint8List k) async {
  ...
  final s3Key =
      'test-pipeline/${date.year}/${p2(date.month)}/${p2(date.day)}/pipe-${date.millisecondsSinceEpoch}.enc';
  ...
  try {
    ...
    try {
      await dec(encrypted, wrongDek);
      fail('Should reject wrong key');
    } catch (_) {
      ok('Wrong key rejected');
    }
  } catch (e) {
    fail('Wrong key test: $e');
  }
  ...
  if (failed > 0) throw Exception('$failed upload pipeline test(s) failed');
}
