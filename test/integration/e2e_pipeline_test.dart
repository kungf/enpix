...
void main() async {
  ...
  Future<Uint8List> encrypt(
    ...
    r.setAll(0, nonce);
    r.setAll(nonce.length, box.cipherText);
    r.setAll(nonce.length + box.cipherText.length, box.mac.bytes);
    return r;
  }
  ...
  Future<Uint8List> wrapKey(Uint8List key, Uint8List kek) async {
    final nonce = rand(24);
    final box =
        await aead.encrypt(key, secretKey: SecretKey(kek), nonce: nonce);
    final r =
        Uint8List(nonce.length + box.cipherText.length + box.mac.bytes.length);
    r.setAll(0, nonce);
    r.setAll(nonce.length, box.cipherText);
  ...
  String sign(
    ...
    final port =
        Uri.parse(endpoint).hasPort ? ':${Uri.parse(endpoint).port}' : '';
    ...
    final canonicalHeaders =
        sorted.map((k) => '$k:${headers[k]!.trim()}').join('\n');
  ...
  final s3Key =
      'e2e-test/${date.year}/${date.month.toString().padLeft(2, '0')}/${date.day.toString().padLeft(2, '0')}/test-${date.millisecondsSinceEpoch}.enc';
  ...
  try {
    ...
    final storedWrappedDek =
        Uint8List.fromList(base64Url.decode(meta['x-amz-meta-dek']!));
